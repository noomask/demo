 ...
 * 弹出请求异常提示
 */
function popErrotMsg(e) {
    if(e.status == 401){
        parent.location.href = basePath + '/resources/pages/user/login.html';
        return
    }
    layer.open({
        title: '请求异常'
 ...
 * @param columns
 */
function loadTreeTable(tableId, url, columns, fn) {
    $('#' + tableId).DataTable({
        'lengthChange': false,
        'searching': false,
        'ordering': false,
 ...
 * @param size 每页显示条数
 */
function loadPageTable(tableId, columns, url, size) {
    $('#' + tableId).DataTable({
        'lengthChange': false,
        'searching': false,
        'ordering': false,
